...
from .forms import ArtisanProfileForm, WasteSellerProfileForm  # Make sure this import is added at the top
from .models import WasteSellerProfile
...
def signup_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            elif user.role == 'waste_seller':
                WasteSellerProfile.objects.create(user=user)
...
@login_required
def waste_seller_profile(request):
    if request.user.role != 'waste_seller':
        return redirect('home')  # Block access if not a waste seller

    waste_seller_profile_instance = request.user.wastesellerprofile
    user_form = CustomUserForm(instance=request.user)
    profile_form = WasteSellerProfileForm(instance=waste_seller_profile_instance)

    if request.method == 'POST':
        user_form = CustomUserForm(request.POST, instance=request.user)
        profile_form = WasteSellerProfileForm(request.POST, request.FILES, instance=waste_seller_profile_instance)

        if user_form.is_valid() and profile_form.is_valid():
            user_form.save()
            profile_form.save()
            return redirect('waste_seller_profile')  # Refresh the page

    return render(request, 'waste_seller_profile.html', {
        'user_form': user_form,
        'profile_form': profile_form,
    })
